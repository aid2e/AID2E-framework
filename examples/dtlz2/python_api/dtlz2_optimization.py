...
- Use a single branch when one computation produces both objectives.
- Use separate branches when each objective has an independent computation.
- Both workflow configurations use the same optimizer and optimization loop.
...
- Batch size of 3 (3 candidates per optimization iteration)
...
def create_single_branch_workflow() -> WorkflowDefinition:
    ...
    One Python evaluator computes and returns both ``f1`` and ``f2``. This
    organization is useful when the objectives share the same computation.
    """
...
def create_separate_branches_workflow() -> WorkflowDefinition:
    ...
    The ``f1_branch`` and ``f2_branch`` each contain their own stage and Python
    evaluator. This organization is useful when the objectives require
...
def run_case_1_single_branch():
    ...
    for batch in range(n_sobol_batches):
        ...
        # Evaluate each candidate through the configured JobLib scheduler
...
if __name__ == "__main__":
    ...
    print("  • Batch size of 3 (3 candidates per optimization iteration)")
    ...
    try:
        ...
        print("  • Batch optimization with 3 candidates per iteration")
        ...
        print("  python examples/dtlz2/python_api/dtlz2_custom_optimizer.py")
