    ...
    def _execute_function(self, job):
        ...
        try:
            ...
            try:
                ...
                # Note: We're already running in a ThreadPoolExecutor, so no need for joblib here
                self.logger.info(f"To run job {job.job_id} function {job.function} with parameters: {job.params}")
                result_dict = job.function(**job.params)
    ...
    def check_job_status(self, job) -> None:
        ...
        if future.done():
            if future.exception():
                ...
                # Get the result and check if it contains an error
