...
def panda_dag_prepare_evaluator(context=None, **kwargs):
    return {"prepared": 5}


def panda_dag_evaluate_from_parent(context=None, **kwargs):
    parent = kwargs["parent_results"]
    return {"f1": parent["prepared"] + 0.1, "f2": parent["prepared"] + 0.2}
...
def test_dag_executor_panda_multistep_scheduler_workflow_returns_objectives(monkeypatch, tmp_path):
    """DAGExecutor should treat one panda_multistep payload as one logical job."""
    state = install_fake_idds(
        monkeypatch,
        result_sequence=[{"prepared": 5}, {"f1": 5.1, "f2": 5.2}],
    )
    ...
    job = JobDefinition(
        ...
        payload={
            ...
            "steps": [
                {"name": "prepare", "python_callable": panda_dag_prepare_evaluator},
                {
                    "name": "evaluate",
                    "python_callable": panda_dag_evaluate_from_parent,
                    "depends_on": ["prepare"],
    ...
    workflow = WorkflowDefinition(
        name="pandaidds_multistep_workflow",
        branches=[BranchDefinition(name="main", stages=[stage])],
        objectives=[
            {"name": "f1", "direction": "minimize"},
            {"name": "f2", "direction": "minimize"},
    ...
    assert objectives == {"f1": 5.1, "f2": 5.2}
    assert executor.global_xcom["dtlz2_multistep:objectives"] == {"f1": 5.1, "f2": 5.2}
    assert state["work_params_history"][1]["op_kwargs"]["parent_results"] == {"prepared": 5}
